    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < nombres.length; i++) {
            ...
            if (edad[i] >= 0 && edad[i] <= 18) {
                PasajeUrbano pMenor = new PasajeMenorEdad(pasajeFijo);
                pasajes.add(pMenor);
                pMenor.establecerPersona(p1);

            } else if (edad[i] > 18 && edad[i] <= 25) {
                PasajeUrbano pUni = new PasajeUniversitario(pasajeFijo);
                pasajes.add(pUni);
                pUni.establecerPersona(p1);

            } else if (edad[i] > 25 && edad[i] < 65) {
                PasajeUrbano pNormal = new PasajeNormal(pasajeFijo);
                pasajes.add(pNormal);
                pNormal.establecerPersona(p1);

            } else if (edad[i] >= 65) {
                PasajeUrbano pTercera = new PasajeTerceraEdad(pasajeFijo);
                pasajes.add(pTercera);
                pTercera.establecerPersona(p1);
